...
def extraer_datos_bestiario():
    ...
    # Capaz de leer tanto el formato viejo como el nuevo Cyberpunk
    bloques = re.split(r'(?=<div[^>]*class="cyber-container"|<div[^>]*style="display: flex;)', content)
    bloques = [b for b in bloques if '<div' in b and ('cyber-container' in b or 'display: flex;' in b)]
...
def generar_html_cyberpunk(nombre, especie, habilidades, debilidades, imagen):
  ...
  <div class="cyber-data-panel">
    <h3 class="cyber-title">Archivo de Entidad</h3>
...
def guardar_como_nueva_entidad():
    ...
    try:
        ...
        limpiar_campos(); cargar_lista_entidades()
...
def migrar_todo_al_nuevo_estilo():
    ...
    respuesta = messagebox.askyesno("Migración", "¿Actualizar los 14 Toy Zombies al nuevo estilo Terminal Cyberpunk?")
    if respuesta:
        nuevo_contenido = ""
...
# --- Interfaz ---
root = tk.Tk()
root.title("Terminal Toy Zombies v4.0")
root.geometry("850x600") 
root.configure(bg="#050505")

frame_main = tk.Frame(root, bg="#050505"); frame_main.pack(fill=tk.BOTH, expand=True, padx=20, pady=20)
...
lbl_total = tk.Label(frame_lista, text="Total: 0", bg="#050505", fg="#888", font=("Consolas", 9)); lbl_total.pack(pady=5)
btn_limpiar = tk.Button(frame_lista, text="NUEVA FICHA", bg="#111", fg="#fff", font=("Consolas", 9, "bold"), relief="flat", command=limpiar_campos); btn_limpiar.pack(fill=tk.X, pady=5)
...
frame_form = tk.Frame(frame_main, bg="#050505"); frame_form.pack(side=tk.LEFT, fill=tk.BOTH, expand=True)
tk.Label(frame_form, text="DATOS DE SISTEMA", font=("Consolas", 14, "bold"), bg="#050505", fg="#ff003c").pack(pady=(0, 15))

def crear_campo(texto):
    f = tk.Frame(frame_form, bg="#050505"); f.pack(fill=tk.X, pady=5)
    tk.Label(f, text=texto, bg="#050505", fg="#00f0ff", font=("Consolas", 10, "bold"), width=15, anchor="w").pack(side=tk.LEFT)
    e = tk.Entry(f, bg="#0a0a0f", fg="#fff", insertbackground="#00f0ff", relief="solid", bd=1); e.pack(side=tk.LEFT, fill=tk.X, expand=True, padx=(10, 0))
    return e
...
frame_botones = tk.Frame(frame_form, bg="#050505"); frame_botones.pack(pady=30, fill=tk.X)
...
limpiar_campos(); cargar_lista_entidades(); root.mainloop()